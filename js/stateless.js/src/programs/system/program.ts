...
type CompressParams = {
    ...
    toAddress: PublicKey;
    /**
     * Owner of the compressed account. If provided, this owner will be able to decompress
     * by signing (owner == authority check). Defaults to SystemProgram.programId if not provided.
     * For EOA wallets to decompress, set this to the payer's public key.
     */
    owner?: PublicKey;
    /**
    ...
    outputStateTreeInfo: TreeInfo;
    /**
     * Optional address params for registering a new address.
     * If provided, the address will be registered in the address tree.
     */
    newAddressParams?: NewAddressParams;
    /**
     * Optional validity proof for new address registration.
     * Required if newAddressParams is provided.
     */
    recentValidityProof?: ValidityProof | null;
};
...
export class LightSystemProgram {
    ...
    static async compress({
        payer,
        toAddress,
        owner = payer,  // Default to payer (user's wallet) if not provided
        lamports,
        outputStateTreeInfo,
        newAddressParams,
        recentValidityProof,
        ...
        // Use owner parameter (defaults to payer) - this is the EOA that will sign during unshield
        // toAddress is the compressed account address (PDA) - used as the address field, not owner
        const outputCompressedAccount = createCompressedAccountLegacy(
            owner,  // ✅ Use owner (EOA) - the wallet that will sign during unshield
            lamports,
            undefined,  // Explicitly no data (data.is_none() check)
            Array.from(toAddress.toBytes()),  // Address as separate parameter (compressed account address/PDA)
        );
        ...
        // Pack new address params if provided (for new address registration)
        let newAddressParamsPacked: any[] = [];
        let remainingAccounts = _remainingAccounts;
        
        if (newAddressParams) {
            const packed = packNewAddressParams([newAddressParams], _remainingAccounts);
            newAddressParamsPacked = packed.newAddressParamsPacked;
            remainingAccounts = packed.remainingAccounts;
        }

        /// Encode instruction data
        const rawInputs: InstructionDataInvoke = {
            proof: recentValidityProof || null,
            inputCompressedAccountsWithMerkleContext:
                packedInputCompressedAccounts,
            outputCompressedAccounts: packedOutputCompressedAccounts,
            relayFee: null,
