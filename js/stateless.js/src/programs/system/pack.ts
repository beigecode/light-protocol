...
export function packCompressedAccounts(
    ...
    inputCompressedAccounts.forEach((account, index) => {
        ...
        // FIX: Ensure owner is a valid PublicKey and data is undefined (not null) for None encoding
        // If data is null, Borsh encodes it as Some(null) instead of None
        // We need to ensure data is undefined or absent for proper None encoding
        const compressedAccountData: any = {
            owner: account.owner, // Must be valid PublicKey
            lamports: account.lamports,
            address: account.address,
        };
        
        // Only include data if it's not null/undefined (for proper None encoding)
        if (account.data !== null && account.data !== undefined) {
            compressedAccountData.data = account.data;
        }
        // If data is null/undefined, omit the field entirely for None encoding
    ...
    outputCompressedAccounts.forEach((account, index) => {
        ...
        // FIX: Ensure data is undefined (not null) for None encoding
        // If data is null, Borsh encodes it as Some(null) instead of None
        // We need to ensure data is undefined or absent for proper None encoding
        const compressedAccountData: any = {
            owner: account.owner,
