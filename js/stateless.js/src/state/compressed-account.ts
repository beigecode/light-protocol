...
export const createCompressedAccountLegacy = (
    ...
    // FIX: Preserve undefined for data field to ensure None encoding (not Some(null))
    // Borsh option() encodes undefined as None, but null as Some(null)
    // This is critical for decompress to work with data-less accounts
    // Type assertion needed because TypeScript type says data: CompressedAccountData | null
    // but we need undefined for proper None encoding
    const result: CompressedAccountLegacy = {
        owner,
        lamports: lamports ?? new BN(0),
        address: address ?? null,
        data: data !== undefined ? data : (null as any), // Type assertion: null for type, but we'll delete it
    };
    // If data is explicitly undefined, omit the field entirely for proper None encoding
    // This ensures Borsh encodes it as None, not Some(null)
    if (data === undefined) {
